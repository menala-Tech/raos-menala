-- RAOS 096 — geofence fallback / pickup_point FK compatibility.
-- raos_geofence_points uses branch.id as `id` for branch-coordinate fallback rows.
-- That value must never be written into raos_attendance.pickup_point_id because
-- the column references pickup_points(id). Keep fallback coordinates for distance
-- validation, but persist pickup_point_id only when the geofence row maps to a real
-- pickup_points record.
