        ...
        {rows.map(row => (
          <div key={row.id} className="rounded-xl border border-gray-100 p-3">
            ...
            <div className="flex justify-between mt-2 text-[10px] text-gray-500">
              <span>{new Date(row.requested_at).toLocaleString('id-ID')}</span>
              <span>{row.status === 'success' ? '✅ AIST Valid' : row.status}</span>
            </div>
            {row.aist_reference && (
              <p className="text-[10px] text-gray-400 mt-1">Ref AIST: {row.aist_reference}</p>
            )}
          </div>
        ))}
      </div>
