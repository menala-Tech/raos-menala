-- ─────────────────────────────────────────────────────────────────────────
-- RAOS 062 — Fix batch F (31 Juli 2026 dinihari):
--   F1 Trigger raos_saldo_after_processed hapus section (2) auto-post
--      ke room Driver cabang. Notifikasi saldo tetap di room Pengisian
--      Saldo saja. Push notif ke staff + progress snapshot ke chat
--      pribadi tetap ada.
--   F3 raos_driver_queue ADD TABLE ke publication supabase_realtime
--      supaya subscription client INSERT/UPDATE fire tanpa reload.
-- ─────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.raos_saldo_after_processed()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $function$
DECLARE
  v_staff_name text; v_branch_name text;
  v_sender_id uuid; v_pribadi_room_id uuid;
  v_snapshot record; v_pin_emoji text; v_pin_label text; v_hint text;
  v_target_display text; v_realisasi_display text;
BEGIN
  ...
  PERFORM raos_dispatch_push(
    ...
    format('Isi saldo Rp%s (%s) sudah diproses admin. Silakan cek di aplikasi.',
      to_char(NEW.nominal, 'FM999,999,999'), NEW.request_no),
    '/riwayat', 'saldo-processed-' || NEW.id, 'pengumuman'
  );

  -- Section (2) auto-chat ke Driver room DIHAPUS per feedback user 31 Juli.

  IF v_sender_id IS NOT NULL AND v_sender_id <> NEW.staff_id THEN
    SELECT s.mode, s.target_val, s.realisasi_val, s.pct
      INTO v_snapshot
      FROM public.raos_saldo_progress_snapshot(NEW.staff_id) s LIMIT 1;
    IF v_snapshot.target_val IS NOT NULL AND v_snapshot.target_val > 0 THEN
      IF v_snapshot.pct >= 100 THEN
        v_pin_emoji := '🟢'; v_pin_label := 'TARGET TERCAPAI';
        v_hint := 'Selamat! Target bulan ini sudah tercapai. Pertahankan.';
      ELSIF v_snapshot.pct >= 50 THEN
        v_pin_emoji := '🟡'; v_pin_label := 'MENUJU TARGET';
        v_hint := 'Semangat! Kejar sisa target sebelum akhir bulan.';
      ELSE
        v_pin_emoji := '🔴'; v_pin_label := 'WASPADA';
      ...
      IF v_pribadi_room_id IS NOT NULL THEN
        ...
        VALUES (
          v_pribadi_room_id, v_sender_id, 'text',
          format(E'%s *%s*\n\n📊 Pencapaian Target Bulan Ini (%s)\n\nTarget: %s\nRealisasi: %s\nPersentase: %s%%\n\n%s\n\n— Sistem Bot RAOS',
            v_pin_emoji, v_pin_label,
            CASE WHEN v_snapshot.mode = 'order' THEN 'Order — Scan Valid' ELSE 'Saldo — Nominal' END,
            v_target_display, v_realisasi_display,
            to_char(v_snapshot.pct, 'FM999,999,990.0'), v_hint)
        );
        PERFORM raos_dispatch_push(
          ARRAY[NEW.staff_id],
          ...
          '/chat', 'saldo-progress-' || NEW.id, 'pengumuman'
        );
      END IF;
    END IF;
  END IF;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
     WHERE pubname = 'supabase_realtime' AND tablename = 'raos_driver_queue'
  ) THEN
    EXECUTE 'ALTER PUBLICATION supabase_realtime ADD TABLE public.raos_driver_queue';
  END IF;
END $$;
