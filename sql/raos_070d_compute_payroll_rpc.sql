...
COMMENT ON FUNCTION public.raos_compute_payroll_month(date) IS 'Hitung Bonus Saldo + Bonus KPI tier per staff, upsert raos_payroll';
COMMENT ON FUNCTION public.raos_random_assign_drivers(uuid, boolean) IS 'Random distribute driver ke staff cabang, management-only';
