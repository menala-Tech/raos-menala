...
export default function RiwayatCabangPage(){
  ...
  return <AppShell>
    ...
    <div className="px-4 py-3 space-y-2">
      ...
      {!loading&&(tab==='semua'||tab==='saldo')&&filteredSaldo.map(r=><div key={r.id} className="card flex gap-3"><div className="p-2 rounded-xl bg-emerald-50"><Wallet size={18}/></div><div className="flex-1"><div className="flex justify-between"><p className="font-bold text-sm">Rp{Number(r.nominal).toLocaleString('id-ID')}</p><span className="text-[10px] uppercase font-bold">{r.is_processed?'paid':r.status}</span></div><p className="text-[11px] text-gray-500">Staff: {staffOf(r) ?? '—'} • {r.request_no}</p><p className="text-[10px] text-gray-400">{r.driver_name ? `Driver: ${r.driver_name} • ` : ''}{new Date(r.requested_at).toLocaleString('id-ID',{timeZone:tz,day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'})} {zone}</p></div></div>)}
